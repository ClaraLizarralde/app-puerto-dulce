  ...
  pedidos.forEach(p => {
    ...
    p.productos.forEach(r => {
      ...
      if(tieneTalle){
        ...
        // Acumular para etiqueta agrupada
    ...
    if(sinTalle.length){
      ...
      for(let i = 0; i < lineas.length; i += CHUNK){
        ...
        etiquetas.push({ nombre: nombre + sufijo, hora, fecha, producto: parte.join('\n'), talle: '', nota: '', conTalle: false, agrupada: true });
      }
    }
  });
  return etiquetas;
...
function etiqSelAll(val){
  ...
  if(val){ _etiqDiasActivos = new Set(dias); }
  else { _etiqDiasActivos = new Set(); }
  _etiqExcluidas = new Set();
  renderEtiquetas();
}

function etiqToggleDia(k){
  if(_etiqDiasActivos.has(k)) _etiqDiasActivos.delete(k);
  else _etiqDiasActivos.add(k);
  _etiqExcluidas = new Set(); // resetear exclusiones al cambiar días
  renderEtiquetas();
}

function etiqToggleCard(uid){
  if(_etiqExcluidas.has(uid)) _etiqExcluidas.delete(uid);
  else _etiqExcluidas.add(uid);
  renderEtiquetas();
}
...
function renderEtiquetas(){
  ...
  for(const k of _etiqDiasActivos) if(!dias.includes(k)) _etiqDiasActivos.delete(k);
...
function imprimirEtiquetas(){
  // Recolectar etiquetas activas de todos los días seleccionados
  let todasEtiquetas = [];
  for(const k of [..._etiqDiasActivos].sort()){
    if(!datos.dias[k]) continue;
    const pedidos = datos.dias[k].pedidos || [];
  ...
  if(!etiquetas.length){ alert('No hay etiquetas seleccionadas para imprimir.'); return; }
