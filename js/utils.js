function fechaKey(d){const y=d.getFullYear(),m=String(d.getMonth()+1).padStart(2,'0'),dd=String(d.getDate()).padStart(2,'0');return`${y}-${m}-${dd}`;}
...
function esCuba(n){return(n||'').toLowerCase()==='cuba';}
...
// ── KEYBOARD NAVIGATION ──
(function(){
  // ── Estado del selector para navegación con flechas ──
  ...
  document.addEventListener('keydown', function(e){
    ...
    if(e.key === 'Escape'){
      if(selectorAbierto){ cerrarSelector(); e.preventDefault(); return; }
      if(modalAbierto){ cerrarModalNP(); e.preventDefault(); return; }
    }
  ...
  // Estilos para el item resaltado con teclado
