/**
 * ================================================================
 * ÍNDICE DE FUNCIONES (PRODUCCION.JS)
 * ================================================================
 * 
 * === INICIALIZACIÓN ===
 * - window._hechoSet               → Almacena estado visual "hecho" por panel (hoy/manana)
 * 
 * === SUBPESTAÑAS DE PRODUCCIÓN ===
 * - showProdTab(id, el)            → Cambia entre subpestañas (hoy, mañana, semana)
 * - renderProduccion()             → Renderiza según pestaña activa
 * 
 * === ESTADO DE ÍTEMS (persistente) ===
 * - getItemKey(pedidoId, rId, idx) → Genera clave única para ítem
 * - getItemEstado(pedidoId, rId, idx) → Obtiene estado (pendiente/en_proceso/listo)
 * - setItemEstado(pedidoId, rId, idx, estado) → Guarda estado del ítem
 * 
 * === HELPERS ===
 * - esSabadoDomingo()              → Retorna true si es sábado o domingo
 * - getDiaOffset(offsetDias)       → Retorna fechaKey con offset de días
 * 
 * === OBTENCIÓN DE PEDIDOS POR RANGO ===
 * - getPedidosRangoHoy()           → Pedidos ST para hoy (incluye anticipados)
 * - getPedidosRangoManana()        → Pedidos ST para mañana (incluye anticipados)
 * 
 * === AGRUPACIÓN Y RENDER ===
 * - buildItemsProduccion(pedidos)  → Agrupa ítems por nombre+talle, suma cantidades
 * - buildProdCard(g, hechoSet, panelId) → Construye tarjeta de producto
 * - buildSeccionProd(titulo, emoji, cls, items, hechoSet, panelId) → Construye sección (pendiente/en_proceso/listo/hecho)
 * - renderPanelProduccion(pedidos, wrapId, vacioId, bannerAntId, panelId) → Renderiza panel completo
 * 
 * === RENDER POR PESTAÑA ===
 * - renderProduccionHoy()          → Renderiza producción del día de hoy
 * - renderProduccionManana()       → Renderiza producción del día de mañana
 * - renderProduccionSemanal()      → Renderiza resumen semanal de ST pendientes
 * 
 * === ACCIONES VISUALES ===
 * - toggleProdChkHecho(chkEl, cardId, panelId) → Marca/desmarca card como "hecho" (visual)
 * - abrirDetalleProduccion(cacheKey) → Abre modal con detalle por unidades
 * - marcarTodoHecho(cacheKey)      → Marca todas las unidades como listas
 * - cerrarDetalle()                → Cierra modal y refresca producción
 * - cambiarEstadoItem(pedidoId, rId, idx, estado, btn) → Cambia estado de unidad individual
 * 
 * ================================================================
 */
...
// Cambia entre subpestañas de producción (hoy, mañana, semana)
...
// Genera clave única para ítem
function getItemKey(pedidoId, rId, idx) {
  return `${pedidoId}_${rId}_${idx}`;
}

// Obtiene estado (pendiente/en_proceso/listo) de un ítem
...
// Retorna fechaKey con offset de días
...
// Obtiene pedidos ST para el rango de HOY
  ...
  Object.entries(datos.dias).forEach(([dKey, dData]) => {
    (dData.pedidos || []).forEach(p => {
      ...
      if (dKey === hoyKey) {
        ...
        const manDate = new Date(hoy);
        manDate.setDate(hoy.getDate() + 1);
...
// Obtiene pedidos ST para el rango de MAÑANA
  ...
  Object.entries(datos.dias).forEach(([dKey, dData]) => {
    (dData.pedidos || []).forEach(p => {
      ...
      if (dKey === manKey && hora >= "14:00") {
        ...
        const pasadoDate = new Date(hoy);
        pasadoDate.setDate(hoy.getDate() + 2);
    ...
    (p.productos || []).filter(r => r.tacc === "s").forEach(r => {
      ...
      if (!grupos[key]) {
        ...
        grupos[key] = {
          ...
          label,
          total: 0,
          estados: { pendiente: 0, en_proceso: 0, listo: 0 },
          unidades: [],
          notas: [],
          categoria: cat
        };
      ...
      for (let i = 0; i < cant; i++) {
        ...
        grupos[key].unidades.push({
          pedidoId: p.id,
          rId: r.id,
          idx: i,
          cliente: p.cliente_input || p.cliente || "Sin nombre",
          nota: r.nota_prod || "",
          estado,
          anticipado: p._anticipado || false
        });
      }
...
// Construye sección (pendiente/en_proceso/listo/hecho)
...
// Renderiza panel completo de producción
...
// Renderiza producción del día de hoy
...
// Renderiza producción del día de mañana
...
function abrirDetalleProduccion(cacheKey) {
  ...
  if (!g) {
    console.error("prod cache miss", cacheKey);
    return;
  }
...
// Marca todas las unidades del detalle como listas
...
// Cierra modal detalle y refresca producción
function cerrarDetalle() {
  document.getElementById("detalle-overlay").classList.add("hidden");
  renderProduccion();
}

// Cambia estado de unidad individual
...
function renderProduccionSemanal() {
  ...
  if (!items.length) {
    wrap.innerHTML = '<div class="vacio" style="padding:16px">Sin pedidos Sin TACC pendientes esta semana. 🎉</div>';
    return;
  }
