// ── CSV ──
function mostrarCSVModal(){document.getElementById('modal-csv').classList.remove('hidden');}
// ══════════════════════════════════════════════════════════════════════════════
...
// ── Hoja PEDIDOS ──────────────────────────────────────────────────────────────
// A4 landscape (277mm útiles): columnas calibradas para entrar en una sola hoja
// ✓(4) + Cuba(4) + Hora(7) + Nombre(24) + Tel(14) + Producto(28) + Talle(8) + Pago(6) + Estado(8) = 103 wch
const COLS_PEDIDO = [
...
// buildPedidosSheet: pedidos mezclados, Matienzo arriba, Cuba agrupado al final
// Una fila por producto. Toda la info se repite en cada fila (sin merges en datos).
// Formato tabla con bordes completos, orientación landscape, ajustado a A4.
function buildPedidosSheet(pedidos, fechaStr, diaNom){
  const aoa      = [];
  const estilos  = [];
  const merges   = [];
  const rowHeights = []; // hpx por fila de datos (se rellena en renderPedido)
  ...
  estilos.push(COLS_PEDIDO.map((c) => celStyle({
    sz:9, bold:true, color: XL.HEADER_TXT, fill: XL.HEADER_BG,
    h: c.ci_center ? "center" : "left", border:false,
    fullBorder: false,
    // custom full border for header
  })));
  // patch header border manually
  estilos[1] = COLS_PEDIDO.map((c) => ({
  ...
  // renderPedido: una fila por producto, todo repetido
  const renderPedido = (p, colorCuba) => {
    ...
    // Pre-rellenar columna ✓ según estado en la app
    ...
    prods.forEach((r) => {
      ...
      // Nota del producto + nota general del pedido
      const notaProd    = (r.nota_prod || "").trim();
      const notaGeneral = (p.notas || "").trim();
      // nota_prod va inline con el producto; nota general solo si no hay nota_prod
      const notaInline  = notaProd || notaGeneral;

      // Celda producto: "Lemon x2" o "Lemon x2\n📝 sin DDL"
      const prodBase = cant > 1 ? `${nom} x${cant}` : nom;
      const prodStr  = notaInline ? `${prodBase}\n📝 ${notaInline}` : prodBase;

      const row = [
        checkVal,  // ✓ pre-rellenado si listo/prod, vacío si pendiente
        cubaChk,   // ● Cuba
        hora,      // Hora
        nombre,    // Nombre
        tel,       // Teléfono
        prodStr,   // Producto + nota inline
        tam,       // Tamaño
        pagado,    // Pago
        estadoApp === "entregado" ? "R" : "",  // R si retirado, vacío para marcar a mano
      ];
      aoa.push(row);

      // Altura de fila: más alta si hay nota
      ...
      const rowEstilos = COLS_PEDIDO.map((c, ci) => {
        ...
        if(ci===5 && notaInline){
          // nota en gris itálica dentro de la misma celda
          base.font.italic = false; // SheetJS no soporta rich text, pero dejamos el color de nota via texto
        }
  ...
  // Separador dorado + bloque Cuba al final
  if(pedCuba.length){
...
function buildProduccionSheet(pedidosTodos, fechaStr, diaNom){
  ...
  // Agrupar todos los productos (sin filtro de tacc, total producción)
  const grupos = {};
  ...
  // Ordenar: nombre asc, talle por tamaño
  ...
  // Agrupar por nombre para saber cuántos items tiene cada uno
  const porNombre = {};
  items.forEach(it => {
  ...
  let fila = 2; // índice en aoa (ya tiene 2 filas)
...
function exportarExcel(soloDia){
  ...
  diasKeys.forEach(key => {
    ...
    // Una sola hoja de pedidos por día: Matienzo arriba, Cuba agrupado al final
    {
...
function exportarCSV(tipo){
  ...
  if(tipo==='pedidos'){
    ...
    const a=document.createElement('a');
    a.href=URL.createObjectURL(new Blob(['\uFEFF'+[cols.join(','),...filas].join('\n')],{type:'text/csv;charset=utf-8'}));
    a.download=`produccion_${diaActual}.csv`;a.click();
  }
}

// ── EXPORTAR CUBA — XLSX y CSV con formato estructurado por producto ──────────
// Formato: sección PEDIDOS (productos de pedidos a Cuba + ventas con llevado>0)
// Columnas: Producto | ch | m | g | (sin talle)
function buildCubaExportData(diaKey){
  ...
  // Recolectar pedidos de la sección "pedidos a Cuba"
  const pedCuba = pedidos.filter(p => esCuba(p.cliente));

  // Agrupar productos de pedidos a Cuba por nombre y talle
  const pedMap = {}; // { nombre: { ch:0, m:0, g:0, sinTalle:0 } }
  pedCuba.forEach(p => {
    (p.productos||[]).forEach(r => {
      const nom = prodNombre(r); if(!nom) return;
  ...
  ventas.forEach(v => {
    ...
    // ventas no tienen talle por ahora → sinTalle
...
function buildCubaXlsx(diaKeys){
  ...
  diaKeys.forEach(key => {
    ...
    const { diaNom, str: fechaStr } = fechaLegible(key);
    const NC = 5; // Producto | ch | m | g | sin talle

    const HEADER_PROD = "Producto";
    const COLS = [
    ...
    function addSection(label, map){
      ...
      aoa.push(totRow);
      ests.push(COLS.map((c,ci)=>celStyle({sz:9,bold:true,fill:XL.TERRACOTA_BG,h:c.center?"center":"left",borderColor:XL.TERRACOTA,fullBorder:true,color:XL.TERRACOTA})));
...
// Exportar Cuba del día actual (llamado desde panel exportar)
function exportarCubaHoy(fmt){
  exportarCubaConFormato([diaActual], fmt);
}

// Exportar Cuba de todos los días disponibles
function exportarCubaTodos(fmt){
  const keys = Object.keys(datos.dias).sort();
  exportarCubaConFormato(keys, fmt);
}

// Exportar Cuba de un día seleccionado (llamado desde selector)
...
// Render selector de días para exportar Cuba historial
...
function exportarCuba(tipo, fmt){
  ...
  // ── Recopilar datos ──
  const pedCuba=getPedidos().filter(p=>esCuba(p.cliente));
  const encargos=getPedidos().filter(p=>!esCuba(p.cliente)&&(p.productos||[]).some(r=>r.tacc!=='s'));
  ...
  function prodItems(p, soloComunes){
    return (p.productos||[])
      ...
      .map(r=>{
        ...
        const _n=Number(r.cantidad);const cant=isNaN(_n)?1:_n;
  ...
  function buildTxt(){
    ...
    if(tipo==='egreso'||tipo==='ambos'){
      ...
      const todosItems=[...pedCuba,...encargos].flatMap(p=>prodItems(p,!esCuba(p.cliente)));
  ...
  const ext = 'txt';
  const a=document.createElement('a');
  a.href=URL.createObjectURL(new Blob([datos_str],{type:mime}));
  a.download=`cuba_${tipo}_${diaActual}.${ext}`;
  a.click();
}
...
function buildRemitoXlsx(diaKey){
  ...
  // Fusionar: sumar por producto y talle
  const merged = {};
  const addMap = (map) => {
...
function importarCatalogoXlsx(input){
  ...
  reader.onload=function(e){
    try{
      ...
      if(errores.length){
        alert('\u26a0 Se saltaron '+errores.length+' fila(s) con tipo inv\u00e1lido:\n'+errores.slice(0,5).join('\n'));
      }
...
function exportarCatalogo(){
  ...
  a.download=`catalogo_puerto_dulce.json`;a.click();
}
...
function restaurarArchivado(id){
  ...
  guardar();renderArchivadosGlobal();renderArchivadosGlobalContent();
...
function exportarArchivadosXlsx(){
  ...
  Object.keys(porFecha).sort().forEach(key => {
    ...
    try {
      ...
      diaNom = f.diaNom; fechaStr = f.str;
    } catch(e){}
